...
function refererTime() {
	...
	if (allSecond <= 0) {
		useAnimate = true;
		getInfo();
		console.log(hasNum,1)
	}
	if (hasNum == false && allSecond == 47) {
		useAnimate = true;
		getInfo();
		console.log(hasNum,47)
	}
	if (hasNum == false && allSecond == 46) {
		useAnimate = true;
		getInfo();
		console.log(hasNum,46)
	}
	if (hasNum == false && allSecond == 45) {
		useAnimate = true;
		getInfo();
		console.log(hasNum,45)
	}
}
//显示隐藏可输入下拉框
			...
			if (data.seasonId != saleSeasonId) {
				...
				if (useAnimate) {
					if (timeAnimate) {	//时间到开启
						hasNum = false; //未开奖
						if(data.lastSeasonId != data.seasonId && 3 >=(data.seasonId - data.lastSeasonId) ){
							$("#openSeasonId").text(data.lastSeasonId);
							lottNumAnimate();//执行动画
						}
					}
				}
			}
			refererBetList(data.bets, false);//刷新我的投注
			refererTraceList(data.traces, false);//追号投注列表
			var lastOpen = data.lastOpen;
            var openSeasonId = data.seasonId;
			if (lastOpen.seasonId != openSeasonId &&  openSeasonId - lastOpen.seasonId == 1) {
				for (var n in lastOpen.nums) {
					...
					$('.lottNumDetailk3').eq(n).find("img").attr("src",'/res/home/images/lott-k3/k3n'+num+'.jpg');
				}
				hasNum = true; //已开奖
				var lotterys = data.lott;
				var group = lotterys.groupId;
				if (Number(lastOpen.seasonId) == Number(data.lastSeasonId) && group == "k3") {
					$("body").stopTime('lottOpen');
					hasNum = false;
					$(".lotteryNum.k3 li.lottNumDetailk3").show();
					$(".lotteryNum.k3 li.lottNumDetailk3None").hide();
				}
				//号码列表
				var openList = "";
				var openListCont = 0;
				if (hs.lottery.pk10) {
					for (var m in data.opens){
						var pkop = data.opens[m];
						openList += '<tr class="beforeLottNumDetail">';
						openList += '<td class="smallFont12">' + pkop.seasonId + '</td>';
						openList += '<td class="fontColorTheme beforeLottNum">';
						for (var k in pkop.nums) {
							openList += '<span> ' + pkop.nums[k] + ' </span> ';
							sum += Number(pkop.nums[k]);
						}
						openList += '</td>';
						openList += '</tr>';
					}
					...
					for (var n in data.opens) {
						...
						openList += '<tr class="beforeLottNumDetail">';
						if (hs.lottery.k3) {
							openList += '<td class="smallFont12">' + op.seasonId.substring(4) + '</td>';
							openList += '<td class="fontColorTheme beforeLottNum k3">';
							for (var n in op.nums) {
								openList += '<span class="k3-' + op.nums[n] + '"> ' + op.nums[n] + ' </span>';
								sum += Number(op.nums[n]);
							}
							openList += '</td>';
							openList += '<td>' + sum + '</td>';
							openList += '<td class="fontColorTheme beforeLottForm kaijiang">';
							if (sum < 11) {
								openList += '<span class="min" style="margin-right: 2px;">小</span>';
							} else {
								openList += '<span class="max" style="margin-right: 2px;">大</span>';
							}
							if (sum % 2 == 0) {
								openList += '<span class="even" style="margin-right: 2px;">双</span>';
							} else {
								openList += '<span class="odd" style="margin-right: 2px;">单</span>';
							}
							openList += '</td>';
						}else {
							openList += '<td>' + op.seasonId.substring(4) + '</td>';
							openList += '<td class="fontColorTheme beforeLottNum">';
							for (var n in op.nums) {
						...
						openList += '</tr>';
						openListCont++;
						if (openListCont >= 10) break;
					}
				}
				$("#openList").html(openList);
				//$("#openList tr:gt(9)").remove();
				//号码形态
				var xtt = "";
...
function lottNumAnimate() {
	...
	$(".lotteryNum.k3 li.lottNumDetailk3").hide();
	$(".lotteryNum.k3 li.lottNumDetailk3None").show();
}
...
function refererBetList(bets, isAppend) {
	...
	for (var n in bets) {
		...
		tr += '<td class="betListLottName overflowEllipsis">' + bet.seasonId + '</td>';
		tr += '<td>￥' + bet.amount + '</td>';
		//盈亏颜色
		var num = 0;
		if (bet.win) {
		...
		if (bet.status == 1) {
			...
			tr += '<td class="fontColorGray">未中奖</td>';
		} else if (bet.status == 0 || bet.status == 6) {
			tr += '<td class="fontColorGray">等待开奖</td>';
		}
		else if (bet.status != 0 || bet.status != 1 || bet.status != 2 || bet.status != 6) {
			tr += '<td class="fontColorGray">0</td>';
		}
		tr += '</tr>';
	}
	if (isAppend) {
		$("#betTable").prepend(tr);	//加到前面
		$("#betTable .beforeLottNumDetail:gt(9)").remove();
	} else {
		...
		ajaxExt({
			...
			callback: function (rel) {
				...
				for (var n in rel) {
					...
					xml += '<tr class="listDetail">';
					xml += '<td>' + (n + 1) + '</td>';
					xml += '<td style="text-align:left;"><input type="checkbox" name="seasonId" value="' + d.seasonId + '" checked="checked" /> <span class="appNumIssue">' + d.seasonId + nowTxt + '</span></td>';
					xml += '<td><input type="text" class="labelCond" name="price" value="' + nowMul + '" /> <span>倍</span></td>';
					xml += '<td data-price="' + nowMoney + '">￥ ' + nowMoney + ' 元</td>';
...
//		var url = "bet?isTrace=0&traceWinStop=0";
		...
		//console.log(' $("#bounsType").val(): ' +  $("#bounsType").val());
		formInputs += '<input type="hidden" name="bounsType" value="0"/>';
		var trs = '';
	...
	$("li.lottNavDetail:eq(" + lottery.contains.length + ")").click();
...
function checkBetIn(betId, amount, currResultAmount) {
	...
	if (parseInt($("#play-" + betId + "-gameCount").text()) <= 0 || Math.sub(amount, currResultAmount) > 0) {
		$awardsLottWarnInfo.html("<p>您本轮的挑战次数/余额不足</p><p>请投注大順彩种，中奖后将再次激活彩中彩</p>");
		colseWin = true;
...
$(function () {
	var num = 0;
	var awidth = $(".lottNavListScoll").width();
	var ulwidth = $(".lottNavListScollBox").width();
	var liwidth = $(".lottNavListScollBox li").width();
	$(".lottNavListScollBox li").click(function () {
		$(this).addClass("active").siblings().removeClass("active");
		var navId = $(this).attr("id");
		window.location.pathname = "/lotts/" + navId + "/index";
		// window.history.pushState(null, null, "/lotts/"+navId+"/index");
	});
	$(".scollRight").click(function () {
		if ((num * liwidth + awidth) < ulwidth) {
			num++;
			if (ulwidth > awidth) {
				$(".lottNavListScollBox").css("left", -(num * liwidth));
			}
		} else if ((num * liwidth + awidth) > ulwidth) {
			num = num;
		}
	});
